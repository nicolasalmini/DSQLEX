defmodule Dsqlex.LexerTest do
  ...
  describe "tokenize/1 - dot-path identifiers" do
    test "simple dot path" do
      assert {:ok, [{:identifier, "config.pricing"}]} = Lexer.tokenize("config.pricing")
    end

    test "multi-level dot path" do
      assert {:ok, [{:identifier, "config.pricing.margin_rate"}]} =
        Lexer.tokenize("config.pricing.margin_rate")
    end
    ...
    test "dot path in expression" do
      ...
      ]} = Lexer.tokenize("base_amount * config.pricing.settlement_rate")
    end
